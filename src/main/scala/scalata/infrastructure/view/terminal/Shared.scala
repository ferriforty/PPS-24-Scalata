 ...
 *
 * Responsibilities <br>
 * • Wire a map `GameControllerState ⇒ Controller[F]` so that the
 * [[scalata.infrastructure.controller.GameEngine GameEngine]]<br>
 * can route the state machine to the correct controller.
 * • Provide a generic `run` loop that repeatedly asks the user for
 * input until the supplied `parse` function succeeds.  <br>
 * • Offer small helper parsers (e.g. `booleanParse`) used by several
 * interactive screens.
 */
object Shared:
   ...
   *
   * @param view shared [[scalata.application.services.GameView GameView]]
   *             used by all controllers for rendering and input
   * @tparam F effect type (e.g. `IO`) in which controllers operate
   * @return a partial function: given the current
   *         [[scalata.domain.util.GameControllerState]],
   ...
   *
   * @param gameView view through which we display prompts / errors and read input
   * @param parse    function that tries to convert raw input `I` to domain value `O`
   * @return the successfully parsed value wrapped in the effect `F`
   */
   ...
   *
   * Accepts only a single-letter **y** or **n** (case-insensitive),
   * trimmed of surrounding white-space.
   *
   * @param raw user-supplied string
   * @return    Some(true)  if `"y"`
   *            Some(false) if `"n"`
   *            None        otherwise
   */

...
/** **Champion-selection screen**.
  ...
  * template, supplying: • an ASCII banner that lists available possible inputs;
  * • a `parse` implementation that maps raw user input.
  *
  * @param view
  *   low-level [[scalata.application.services.GameView GameView]] through which
  *   all rendering and input occur (dependency-injected so that we stay
  *   UI-agnostic).
  * @tparam F
  *   effect type (e.g. `IO`)
  */

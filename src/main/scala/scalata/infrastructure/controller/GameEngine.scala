...
/** Orchestrates high-level state transitions of the game.
  *
  * <ul> <li>Picks the controller that matches the current
  * <code>GameControllerState</code>.</li> <li>Delegates one step to that
  * controller.</li> <li>Reads the returned <code>GameResult</code> and, if
  * successful, recurses with the next phase and updated
  * <code>GameBuilder</code>.</li> <li>Ends with <code>ExitCode.Success</code>
  * when any controller returns an error (e.g. player quit).</li> </ul>
  *
  * @tparam F
...
class GameEngine[F[_]: Sync]:
  ...
  /** Runs the main finite-state machine loop.
    *
    * @param gamePhaseService
    *   holder of the current phase; mutated via <code>transitionTo</code>
    * @param gameBuilder

...
/** Use-case that executes a **full game turn**:
  *
  * <h4>Flow</h4> <ol> <li><b>Parse player command</b> &nbsp;→&nbsp; run the
  * matching player use-case.</li> <li><b>On success</b> &nbsp;→&nbsp; apply two
  * enemy phases in order: <ul> <li><code>EnemyAttackUseCase</code></li>
  * <li><code>EnemyMovementUseCase</code></li> </ul> </li> <li><b>Error
  * commands</b> (<i>quit / undo / help</i>) return a corresponding
  * <code>GameError</code>.</li> </ol>
  */
class GameRunningUseCase:
  ...
  /** Execute a single turn:
    ...
    * @tparam F
    *   effect type with a [[cats.Monad]] instance (e.g. <code>IO</code>)
    * @return
    *   a [[scalata.domain.util.GameResult]] containing the updated

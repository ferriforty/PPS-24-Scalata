...
/** Executes the **enemy attack phase**.
  *
  * <h4>Algorithm</h4> <ul> <li>Fetch the current <code>Player</code> from the
  * session.</li> <li>For every <i>alive</i> enemy in the room: <ul> <li>Compute
  * the four-tile neighbourhood around the enemy
  * (<code>neighboursFiltered</code>).</li> <li>If the player’s position lies in
  * that neighbourhood, apply <code>enemy.attack(player)</code>.</li> </ul>
  * </li> <li>Return the player after all damage has been accumulated (purely
  * functional fold).</li> </ul>
  *

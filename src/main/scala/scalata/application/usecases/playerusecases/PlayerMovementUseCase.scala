...
/** Handles a single **movement command** issued by the player.
  *
  * <h4>Decision tree</h4> <ul> <li><b>Simple step</b> – if the target tile lies
  * inside the current room and is free of living enemies/items, update the
  * player’s position.</li> <li><b>Door transition</b> – when the tile matches
  * the current door and the room contains an exit in that direction, move the
  * player to the neighbouring room, advance <code>GameState.currentRoom</code>
  * and push any enemy standing on the entrance one tile inward.</li>
  * <li><b>Invalid</b> – in all other cases return
  * <code>GameError.InvalidInput</code>.</li> </ul>
  */
class PlayerMovementUseCase
  ...
  /** Attempt to move the player one tile towards <code>direction</code>.
    ...
    * @return
    *   updated session wrapped in <code>GameResult.success</code> or a

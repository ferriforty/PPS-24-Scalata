...
/** Handles the **“use item”** command.
  *
  * <ul> <li>Looks up the item by <code>itemName</code> in the player’s
  * inventory.</li> <li>Delegates to the <code>Usable</code> type-class to apply
  * the effect (<i>Potion</i>, <i>Weapon</i>, <i>Dust</i>, …).</li> <li>Updates
  * the player inside the <code>World</code> and returns a
  * <code>GameResult.success</code> wrapping the new
  * <code>GameSession</code>.</li> <li>If the player does not own the item,
  * returns <code>GameResult.Error(GameError.ItemNotOwned)</code>.</li> </ul>
  */
class PlayerInventoryUseCase
  ...
  /** Apply the effect of the requested item.
    ...
    * @return
    *   updated session in <code>GameResult.success</code> or
    *   <code>ItemNotOwned</code> error
    */

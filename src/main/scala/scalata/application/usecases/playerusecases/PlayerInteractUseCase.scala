...
/** Handles the **“interact”** command (pick up / open / read whatever
 * lies on the tile immediately in front of the player).
 *
 * <h4>Flow</h4>
 * <ul>
 * <li>Compute the target coordinate by adding <code>direction.vector</code>
 * to the player’s current position.</li>
 * <li>If an item is present, delegate to its
 * <code>interact(GameSession)</code> method and propagate the result.</li>
 * <li>If the tile is empty, return
 * <code>GameResult.error(GameError.ItemNotPresent)</code>.</li>
 * </ul>
 *
...
class PlayerInteractUseCase
   ...
   *
   * @param direction   cardinal direction chosen by the player
   * @param gameSession current immutable snapshot
   * @return            a successful <code>GameResult</code> with the updated
   *                    session, or an error signalling that no item is there
   */

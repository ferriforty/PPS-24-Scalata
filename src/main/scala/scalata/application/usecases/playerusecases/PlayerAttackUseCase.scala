 ...
 * <h4>Algorithm</h4>
 * <ul>
 * <li>Compute a *reach vector* whose length equals the player’s
 * <code>reach</code> stat.</li>
 * <li>Build the inclusive line of points from the player’s position to the
 * target tile (<code>rangeTo</code>).</li>
 * <li>For every enemy in the current room:
 * <ul>
 * <li>If the enemy is <i>alive</i> **and** its position lies on the
 * line, apply <code>player.attack</code>.</li>
 * </ul>
 * </li>
 * <li>Return an updated <code>GameSession</code> wrapped in
 * <code>GameResult.success</code>.</li>
 * </ul>
 *
 * <b>Failure modes</b>
 * If the room referenced by <code>GameState.currentRoom</code> is missing, an
...
class PlayerAttackUseCase
   ...
   * @param gameSession immutable snapshot before the attack
   * @return updated snapshot wrapped in a successful <code>GameResult</code>
   */

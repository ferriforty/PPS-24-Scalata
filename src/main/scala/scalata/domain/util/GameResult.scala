 ...
 * <h4>Cases</h4>
 * <ul>
 * <li><code>Success[T]</code> – carries a value of type <code>T</code> and an optional user-facing message.</li>
 * <li><code>Error</code> – wraps a non-recoverable <code>GameError</code>.</li>
 * </ul>
 *
 * Helper constructors <code>success</code> and <code>error</code> make call-sites concise.
 */
object GameResult:

  /** Successful outcome.
   *
   * @param value   payload returned by the operation
   * @param message optional note to show in the UI (help text, info, …)
   */

 ...
 *
 * <ul>
 * <li><b>pick</b> removes the item from the current room and adds it to
 * the player’s inventory, returning an updated
 * [[scalata.domain.world.GameSession GameSession]].</li>
 * <li>If the room referenced by <code>GameState.currentRoom</code> cannot be
 * found (world corruption), the method throws
...
trait Pickable:
   ...
   * @param gameSession current session snapshot
   * @return a new session with world and player updated
   */

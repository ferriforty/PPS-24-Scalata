...
/** Type-class describing an item that can be **used** by an entity.
  *
  * <h4>Contract</h4> <ul> <li><b>A</b> – concrete item subtype.</li>
  * <li><b>E</b> – entity subtype that can own / use the item.</li>
  * <li><code>use</code> returns a <u>new</u> instance of the owner reflecting
  * the item’s effect (pure, no mutation).</li> </ul>
  */
...
/** Inline helper that tries to use a runtime item if a matching
  ...
  *
  * <p>The method performs an inline pattern-match on the concrete item type
  * and, through <code>summonInline</code>, obtains the given instance for
  * <code>Potion</code>, <code>Dust</code> or <code>Weapon</code>. Unsupported
  * items yield <code>None</code>.</p>
  */
object Usable:
